    ...
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-white flex flex-col">
      <div className="w-full max-w-lg mx-auto p-6 flex-1 flex flex-col animate-fade-in">
        ...
        <div className="flex-1 space-y-3 mb-8">
          {currentStep.isFreeInput ? (
            ...
            currentStep.options?.map((option) => (
              <button key={option} onClick={() => updateProfile(currentStep.field, option)} className={`w-full p-4 rounded-xl border-2 transition-all text-left font-medium ${profile[currentStep.field] === option ? "border-orange-500 bg-orange-50 text-orange-700 shadow-md" : "border-gray-200 bg-white text-gray-700 hover:border-orange-200 hover:bg-orange-50/50"}`}>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleNext} disabled={!canProceed()} className={`flex-1 py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${canProceed() ? "bg-gradient-to-r from-orange-400 to-orange-500 text-white shadow-md hover:shadow-lg active:scale-95" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}>
