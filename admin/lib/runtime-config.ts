...
export function assertAdminRuntimeConfig(env: NodeJS.ProcessEnv = process.env): void {
  ...
  try {
    assertAdminRuntimeConfigStrict(env);
  } catch (error) {
    console.warn("[runtime-config] 管理者設定チェックをスキップしました:", error instanceof Error ? error.message : error);
  }
}

function assertAdminRuntimeConfigStrict(env: NodeJS.ProcessEnv): void {
