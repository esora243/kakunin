...
import { updateJobCategory } from "@/lib/master-data";
...
function resolveIdFromRequest(request: Request): string {
  ...
  if (!raw) throw new ValidationError("Job category id must be a valid string", "invalid_id");
  try {
    return decodeURIComponent(raw);
  } catch {
    throw new ValidationError("Job category id must be a valid string", "invalid_id");
  }
}

// Rename only, per docs/admin-management-app-spec.md Master Data row for
// `job_categories` ("No create/deactivate at launch"). Renamed labels affect
// the Jobs list/detail pages, so invalidate the "jobs" tag even though the
// spec's cache table doesn't list this row explicitly.
export const PATCH = adminApiRoute("owner", async (identity, request) => {
  ...
  const { value: { before, after }, cacheResult } = await mutateWithPublicCacheInvalidation(identity.adminId, async (client) => {
    const result = await updateJobCategory(client, id, name, identity.adminId);

    await writeAuditLog(client, {
      actorAdminId: identity.adminId,
      action: "master_data.job_category.update",
      resourceType: "job_categories",
      resourceId: id,
    ...
    return result;
  }, { resourceType: "job_categories", resourceId: id, tags: ["jobs"] });

  return { jobCategory: after, before, cacheWarning: !cacheResult.ok };
});
