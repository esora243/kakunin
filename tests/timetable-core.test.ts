...
test("mapTimetableClass rejects malformed, weekend, and out-of-contract schedules", () => {
  for (const schedule of [
    ...
    { day: "月", period: 7 },
    { weekday: "火", period: 2 },
    { day: "月", period: 1, starts_at: 840 },
    { day: "月", period: 1, ends_at: { hour: 10, minute: 10 } },
  ]) {
    assert.throws(() => mapTimetableClass(row({ schedule })), /integrity check failed/);
  }
});
